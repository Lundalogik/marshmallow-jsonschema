...
class ReactJsonSchemaFormJSONSchema(JSONSchema):
    ...
    json_schema, uischema = json_schema_obj.dump_uischema(MySchema)
    """

    def dump_with_uischema(self, obj, *args, many=None):
    ...
    def dump_uischema(self, obj, *args, many=None):
        """
    ...
    def _dump_uischema_iter(self, obj, *args, many=None):
        """
        This is simply implementing a Dictionary Iterator for
        ReactJsonSchemaFormJSONSchema.dump_uischema
        """

        for k, v in getattr(obj.Meta, "react_uischema_extra", {}):
            yield k, v

        for field_name, field in obj.fields.items():
            metadata = field.metadata.get("metadata", {})
            yield field_name, {k: v for k, v in metadata if k.startswith("ui:")}
